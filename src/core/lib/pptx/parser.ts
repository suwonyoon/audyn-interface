...
import type { Presentation, Slide, Theme, ThemeColors, ThemeFonts } from '@core/types'
import { parseSlide } from './slideParser'
import { extractMedia } from './imageParser'
import { parseNotesSlide } from './notesParser'
import { emuToPixels } from '@core/lib/utils/emuConversion'
import { generateId, ensureArray } from '@core/lib/utils/xmlUtils'
...
/** Input type for PPTX parsing - supports both File and ArrayBuffer */
export type PPTXInput = File | ArrayBuffer

/** Options for parsing PPTX */
export interface ParsePPTXOptions {
  /** Name to use when input is ArrayBuffer (defaults to 'Presentation') */
  name?: string
}

/**
 * Parse a PPTX file from either a File object or ArrayBuffer
 */
export async function parsePPTX(
  ...
  // Determine presentation name
  const name = input instanceof File
    ? input.name.replace('.pptx', '')
